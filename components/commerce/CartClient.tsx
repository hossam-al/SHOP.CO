  ...
  return (
    <section className="container-page py-8">
      <div className="mb-8 text-black/60">
        {site("home")} <span aria-hidden="true">/</span> <span className="text-black">{site("cart")}</span>
      </div>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1fr_505px]">
              ...
              <article key={product.id} className="grid gap-4 border-b border-black/10 py-5 last:border-b-0 sm:grid-cols-[124px_1fr_auto]">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm">
                    {t("size")}: <span className="text-black/60">{t("large")}</span>
                  </p>
                  <p className="mt-1 text-sm">
                    {t("color")}: <span className="text-black/60">{t("blue")}</span>
                  </p>
                ...
                <div className="flex flex-col items-end justify-between gap-5">
                  ...
                  <div className="flex items-center gap-2 rounded-full bg-[var(--background-soft)] px-4 py-3">
                    <button onClick={() => updateQuantity(product.id, quantity - 1)} aria-label={t("decreaseQuantity")}>
                      <Minus size={16} />
                    </button>
                    <span className="min-w-8 text-center font-bold">{quantity}</span>
                    <button onClick={() => updateQuantity(product.id, quantity + 1)} aria-label={t("increaseQuantity")}>
                      <Plus size={16} />
                    </button>
                  </div>
                </div>
        ...
        <aside className="h-fit rounded-[20px] border border-black/10 p-6">
          ...
          <Link className="btn-primary mt-6 w-full" href="/CHEKOUT">
            {site("checkout")} →
          </Link>
