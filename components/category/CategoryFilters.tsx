...
export function CategoryFilters({
  ...
  const content = (
    <>
      ...
      <FilterTab id="price" title={t("price")}>
      ...
      <FilterTab id="colors" title={t("colors")}>
      ...
      <FilterTab id="size" title={t("size")}>
        <div className="flex max-w-full flex-wrap gap-2">
          {sizes.map((size) => {
      ...
      <FilterTab id="dressStyle" title={t("dressStyle")} isLast>
        <div className="grid gap-1 text-sm text-black/70">
          {categories.map((category) => {
            ...
            return (
              <Link
                key={category.slug}
                href={category.slug === "casual" ? "/PLP" : `/PLP?category=${category.slug}`}
    ...
    <>
      ...
      {showMobileButton && mobileOpen ? (
        <button
          ...
          aria-label={t("closeFiltersOverlay")}
        />
      ) : null}
