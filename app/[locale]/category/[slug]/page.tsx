...
export default async function CategoryPage({
  ...
  const { locale, slug } = await params;
  const { sort, color, size, min, max, page } = await searchParams;
  const category = getCategory(slug);
  if (!category) notFound();

  const t = await getTranslations({ locale, namespace: "category" });
  const name = locale === "ar" ? category.name_ar : category.name_en;
  const categoryItems = getProductsByCategory(slug);
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 text-black/60">
        Home / <span className="text-black">{name}</span>
      </div>
        ...
        <main className="min-w-0 flex-1">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3 text-black/60 sm:justify-end">
              <span className="text-sm">
                Showing {showingStart}-{showingEnd} of {items.length} Products
              </span>
              <CategoryFilters
                colors={colors}
                sizes={sizes}
          ...
          {paginatedItems.length ? (
            ...
            <div className="mt-8 rounded-2xl border border-black/10 bg-white p-8 text-center">
              ...
              <p className="mt-2 text-sm text-black/60">Try clearing filters or choosing another option.</p>
            </div>
          )}
          ...
          <div className="mt-10 flex items-center justify-between gap-3 border-t border-black/10 pt-5">
            {currentPage > 1 ? (
              ...
              <span className="btn-secondary !px-5 opacity-50">Previous</span>
            )}
            ...
            {currentPage < totalPages ? (
              ...
              <span className="btn-secondary !px-5 opacity-50">Next</span>
            )}
          </div>
        </main>
      </div>
